...
public class Subsystem {
    public static void main(String[] arg) {
        try {
            ...
            /*
             * channel.setInputStream(System.in); ((ChannelSubsystem)channel).setErrStream(System.err); InputStream in =
             * channel.getInputStream(); channel.connect();
             * 
             * byte[] tmp=new byte[1024]; while(true){ while(in.available()>0){ int i=in.read(tmp, 0, 1024);
             * if(i<0)break; System.out.print(new String(tmp, 0, i)); } if(channel.isClosed()){
